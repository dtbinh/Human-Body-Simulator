...
static void lib_link_pose(FileData *fd, Main *bmain, Object *ob, bPose *pose)
{
	...
	if (ob->proxy) {
		...
		if (pose->proxy_act_bone[0]) {
			ArmatureElement *bone = BKE_armature_find_element_name(arm, pose->proxy_act_bone);
			if (bone)
				arm->act_element = bone;
		}
	}
	...
	for (pchan = pose->chanbase.first; pchan; pchan=pchan->next) {
		...
		pchan->bone = BKE_armature_find_element_name(arm, pchan->name);
...
static void direct_link_armatureelements(FileData *fd, ArmatureElement *elem)
{
    ArmatureElement *child;

    elem->parent = newdataadr(fd, elem->parent);
    elem->prop = newdataadr(fd, elem->prop);
    elem->data = newdataadr(fd, elem->data);
    IDP_DirectLinkGroup_OrFree(&elem->prop, (fd->flags & FD_FLAGS_SWITCH_ENDIAN), fd);

    elem->flag &= ~ELEMENT_DRAW_ACTIVE;

    link_list(fd, &elem->childbase);

    for (child = elem->childbase.first; child; child = child->next)
        direct_link_armatureelements(fd, child);
}

static void convert_bonelist_to_elementlist(ListBase *dstbase, ListBase *srcbase)
{
    Bone            *bone;
    ArmatureElement *elem;

    for (bone = srcbase->first; bone; bone = bone->next)
    {
        elem = MEM_callocN(sizeof(ArmatureElement), "armatureelement");
        elem->data = MEM_callocN(sizeof(BoneData), "bonedata");
        elem->type = AE_BONE;

        BLI_strncpy(elem->name, bone->name, MAXBONENAME);

        elem->roll = bone->roll;
        copy_v3_v3(elem->head, bone->head);
        copy_v3_v3(elem->tail, bone->tail);
        copy_m3_m3(elem->AE_mat, bone->bone_mat);

        elem->flag = bone->flag;

        copy_v3_v3(elem->arm_head, bone->arm_head);
        copy_v3_v3(elem->arm_tail, bone->arm_tail);
        copy_m4_m4(elem->arm_mat, bone->arm_mat);
        elem->arm_roll = bone->arm_roll;

        elem->rad_head = bone->rad_head;
        elem->rad_tail = bone->rad_tail;

        elem->xwidth = bone->xwidth;
        elem->length = bone->length;
        elem->zwidth = bone->zwidth;

        elem->layer = bone->layer;
        elem->segments = bone->segments;

        ((BoneData*)elem->data)->dist = bone->dist;
        ((BoneData*)elem->data)->weight = bone->weight;
        ((BoneData*)elem->data)->ease1 = bone->ease1;
        ((BoneData*)elem->data)->ease2 = bone->ease2;

        BLI_addtail(dstbase, elem);

        convert_bonelist_to_elementlist(&elem->childbase, &bone->childbase);
    }
}
...
static void direct_link_armature(FileData *fd, bArmature *arm)
{
	...
	for (elem = arm->elementbase.first; elem; elem = elem->next) {
        direct_link_armatureelements(fd, elem);
	}

    /* Convert data from old versions to new version */
	if (BLI_listbase_is_empty(&arm->elementbase)) {
        convert_bonelist_to_elementlist(&arm->elementbase, &arm->bonebase);
	}
    /* All bones are stored in elementbase */
	BKE_armature_bonelist_free(&arm->bonebase);

	arm->act_element = newdataadr(fd, arm->act_element);
