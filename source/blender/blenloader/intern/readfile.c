...
static void direct_link_muscles(FileData *fd, Muscle *muscle)
{
    Muscle *child;

    muscle->parent = newdataadr(fd, muscle->parent);
    muscle->prop = newdataadr(fd, muscle->prop);
    IDP_DirectLinkGroup_OrFree(&muscle->prop, (fd->flags & FD_FLAGS_SWITCH_ENDIAN), fd);

    muscle->flag &= ~MUSCLE_DRAW_ACTIVE;

    link_list(fd, &muscle->childbase);

    for (child=muscle->childbase.first; child; child=child->next)
        direct_link_muscles(fd, child);
}
...
static void direct_link_armature(FileData *fd, bArmature *arm)
{
	...
	for (muscle = arm->musclebase.first; muscle; muscle = muscle->next) {
        direct_link_muscles(fd, muscle);
	}

	arm->act_bone = newdataadr(fd, arm->act_bone);
	arm->act_edbone = NULL;

	arm->act_muscle = newdataadr(fd, arm->act_muscle);
...
static void direct_link_pose(FileData *fd, bPose *pose)
{
	...
	for (pmuscle = pose->musclebase.first; pmuscle; pmuscle=pmuscle->next) {
        pmuscle->muscle = NULL;
        pmuscle->parent = newdataadr(fd, pmuscle->parent);
        pmuscle->child = newdataadr(fd, pmuscle->child);

        pmuscle->prop = newdataadr(fd, pmuscle->prop);
        IDP_DirectLinkGroup_OrFree(&pmuscle->prop, (fd->flags & FD_FLAGS_SWITCH_ENDIAN), fd);
	}
