...
static void write_pose(WriteData *wd, bPose *pose)
{
	...
	for (chan=pose->chanbase.first; chan; chan=chan->next) {
		...
		if (chan->bone)
			chan->selectflag= chan->bone->flag & ELEMENT_SELECTED; /* gets restored on read, for library armatures */
	...
	while (ob) {
		if (ob->id.us>0 || wd->current) {
			...
			if (ob->type == OB_ARMATURE) {
				...
				if (arm && ob->pose && arm->act_element) {
					BLI_strncpy(ob->pose->proxy_act_bone, arm->act_element->name, sizeof(ob->pose->proxy_act_bone));
				}
			}
...
static void write_armatureelement(WriteData *wd, ArmatureElement *elem)
{
    ArmatureElement *celem;

    writestruct(wd, DATA, "ArmatureElement", 1, elem);
    switch (elem->type) {
    case(AE_BONE):
        writestruct(wd, DATA, "BoneData", 1, elem->data);
    case(AE_MUSCLE):
        writestruct(wd, DATA, "MuscleData", 1, elem->data);
    }

    if (elem->prop)
        IDP_WriteProperty(elem->prop, wd);

    celem = elem->childbase.first;
    while (celem) {
        write_armatureelement(wd, celem);
        celem = celem->next;
    }
}

static void convert_bonedata(ListBase *dstbase, ListBase *srcbase)
{
    Bone            *bone;
    ArmatureElement *elem;

    BKE_armature_bonelist_free(dstbase);

    for (elem = srcbase->first; elem; elem = elem->next) {
        if (elem->type == AE_BONE) {
            bone = MEM_callocN(sizeof(Bone), "writebone");
            BLI_strncpy(bone->name, elem->name, MAXBONENAME);

            bone->roll = elem->roll;
            copy_v3_v3(bone->head, elem->head);
            copy_v3_v3(bone->tail, elem->tail);
            copy_m3_m3(bone->bone_mat, elem->AE_mat);

            bone->flag = elem->flag;

            copy_v3_v3(bone->arm_head, elem->arm_head);
            copy_v3_v3(bone->arm_tail, elem->arm_tail);
            copy_m4_m4(bone->arm_mat, elem->arm_mat);
            bone->arm_roll = elem->arm_roll;

            bone->rad_head = elem->rad_head;
            bone->rad_tail = elem->rad_tail;

            bone->xwidth = elem->xwidth;
            bone->length = elem->length;
            bone->zwidth = elem->zwidth;

            bone->layer = elem->layer;
            bone->segments = elem->segments;

            bone->dist = ((BoneData*)elem->data)->dist;
            bone->weight = ((BoneData*)elem->data)->weight;
            bone->ease1 = ((BoneData*)elem->data)->ease1;
            bone->ease2 = ((BoneData*)elem->data)->ease2;

            BLI_addtail(dstbase, bone);

            convert_bonedata(&bone->childbase, &elem->childbase);
        }
    }
}
...
static void write_armatures(WriteData *wd, ListBase *idbase)
{
	...
	while (arm) {
		if (arm->id.us>0 || wd->current) {
            /* Convert elementbase bones to struct bones */
            /* For backwards compatibility */
            convert_bonedata(&arm->bonebase, &arm->elementbase);
			...
			elem = arm->elementbase.first;
			while (elem) {
                write_armatureelement(wd, elem);
                elem = elem->next;
			}
