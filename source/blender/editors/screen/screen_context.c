...
//							if ((flipbone) && !(flipbone->flag & BONE_SELECTED))
						if (EELEMENT_EDITABLE(ebone)) {
							CTX_data_list_add(result, &arm->id, &RNA_EditArmatureElement, ebone);

							if ((flipbone) && !(flipbone->flag & ELEMENT_SELECTED))
								CTX_data_list_add(result, &arm->id, &RNA_EditArmatureElement, flipbone);
						}
					}
					else {
		...
		if (obact && obact->type == OB_ARMATURE) {
			...
			if (arm->edel) {
				if (arm->act_edelement) {
					CTX_data_pointer_set(result, &arm->id, &RNA_EditArmatureElement, arm->act_edelement);
					return 1;
				}
			}
			else {
...
//					CTX_data_pointer_set(result, &arm->id, &RNA_Bone, arm->act_bone);
				if (arm->act_element) {
					CTX_data_pointer_set(result, &arm->id, &RNA_ArmatureElement, arm->act_element);
					return 1;
				}
			}
		}
