...
static int pose_flip_names_exec(bContext *C, wmOperator *UNUSED(op))
{
	...
	CTX_DATA_BEGIN (C, bPoseChannel *, pchan, selected_pose_bones)
	{
		...
		ED_armature_element_rename(arm, pchan->name, name_flip);
	}
	CTX_DATA_END;
...
static int pose_autoside_names_exec(bContext *C, wmOperator *op)
{
	...
	CTX_DATA_BEGIN (C, bPoseChannel *, pchan, selected_pose_bones)
	{
		...
		if (bone_autoside_name(newname, 1, axis, pchan->bone->head[axis], pchan->bone->tail[axis]))
			ED_armature_element_rename(arm, pchan->name, newname);
	}
	CTX_DATA_END;
...
static int pose_bone_layers_exec(bContext *C, wmOperator *op)
{
	...
	CTX_DATA_BEGIN (C, bPoseChannel *, pchan, selected_pose_bones)
	{
		...
		RNA_pointer_create((ID *)ob->data, &RNA_ArmatureElement, pchan->bone, &ptr);
		RNA_boolean_set_array(&ptr, "layers", layers);
	}
	CTX_DATA_END;
...
static int armature_bone_layers_exec(bContext *C, wmOperator *op)
{
	...
	{
		/* get pointer for pchan, and write flags this way */
		RNA_pointer_create((ID *)arm, &RNA_EditArmatureElement, ebone, &ptr);
		RNA_boolean_set_array(&ptr, "layers", layers);
	}
	CTX_DATA_END;
...
static int hide_unselected_pose_bone_cb(Object *ob, ArmatureElement *bone, void *UNUSED(ptr))
{
	...
	if (arm->layer & bone->layer) {
		/* hrm... typo here? */
		if ((bone->flag & ELEMENT_SELECTED) == 0) {
			bone->flag |= ELEMENT_HIDDEN_P;
			if (arm->act_element == bone)
				arm->act_element = NULL;
		}
...
static int pose_hide_exec(bContext *C, wmOperator *op)
{
	...
	if (RNA_boolean_get(op->ptr, "unselected"))
		bone_looper(ob, arm->elementbase.first, NULL, hide_unselected_pose_bone_cb);
	else
		bone_looper(ob, arm->elementbase.first, NULL, hide_selected_pose_bone_cb);
...
static int pose_reveal_exec(bContext *C, wmOperator *UNUSED(op))
{
	...
	bone_looper(ob, arm->elementbase.first, NULL, show_pose_bone_cb);
