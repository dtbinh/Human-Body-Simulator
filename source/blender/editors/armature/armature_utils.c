...
void ED_armature_sync_selection(ListBase *edel)
{
...
void element_free(bArmature *arm, EditArmatureElement *bone)
{
    ...
    if (BLI_findindex(arm->edel, bone) != -1)
        BLI_freelinkN(arm->edel, bone);
}
...
void ED_armature_edit_element_remove(bArmature *arm, EditArmatureElement *exBone)
{
    ...
    for (curBone = arm->edel->first; curBone; curBone = curBone->next) {
 ...
 */
EditArmatureElement *ED_armature_element_find_name(const ListBase *edel, const char *name)
{
	return BLI_findstring(edel, name, offsetof(EditArmatureElement, name));
}
 ...
 */
EditArmatureElement *ED_armature_element_get_mirrored(const ListBase *edel, EditArmatureElement *eel)
{
	...
	if (!STREQ(name_flip, eel->name)) {
		return ED_armature_element_find_name(edel, name_flip);
	}
        ...
        for (curBone = arm->edel->first; curBone; curBone = curBone->next) {
            if (arm->layer & curBone->layer) {
                if (curBone->flag & ELEMENT_SELECTED) {
                    ebone_mirr = ED_armature_element_get_mirrored(arm->edel, curBone);
                    if (ebone_mirr)
        ...
        for (curBone = arm->edel->first; curBone; curBone = curBone->next) {
            if (arm->layer & curBone->layer) {
                if (curBone->flag & (ELEMENT_SELECTED | ELEMENT_ROOTSEL | ELEMENT_TIPSEL)) {
                    EditArmatureElement *ebone_mirr = ED_armature_element_get_mirrored(arm->edel, curBone);
...
void transform_armature_mirror_update(Object *obedit)
{
    ...
    for (ebo = arm->edel->first; ebo; ebo = ebo->next) {
        ...
        if (ebo->flag & (ELEMENT_TIPSEL | ELEMENT_ROOTSEL)) {
            eboflip = ED_armature_element_get_mirrored(arm->edel, ebo);
            ...
            if (eboflip) {
                ...
                if (ebo->flag & ELEMENT_TIPSEL) {
                    ...
                    for (children = arm->edel->first; children; children = children->next) {
...
EditArmatureElement *make_elementList(ListBase *edList, ListBase *elements, EditArmatureElement *parent, ArmatureElement *actElement)
{
	...
	eElement = MEM_callocN(sizeof(EditArmatureElement), "make_editbone");
		...
		case(AE_BONE):
			...
			break;
		case(AE_MUSCLE):
		    eElement->data = MEM_callocN(sizeof(EditBoneElement), "make_editmuscledata");
			break;
		}
...
void ED_armature_from_edit(bArmature *arm)
{
    ...
    for (eElem = arm->edel->first; eElem; eElem = neElem) {
        ...
        if (len <= 0.000001f) {  /* FLT_EPSILON is too large? */
            ...
            /*	Find any bones that refer to this bone	*/
            for (fBone = arm->edel->first; fBone; fBone = fBone->next) {
    ...
    for (eElem = arm->edel->first; eElem; eElem = eElem->next) {
        ...
        switch(eElem->type) {
            case(AE_BONE):
                ...
                break;
            case(AE_MUSCLE):
                newElem->data = MEM_callocN(sizeof(MuscleData), "muscledata");
                break;
        }
	...
	fix_bonelist_roll(&arm->elementbase, arm->edel);
...
void ED_armature_to_edit(bArmature *arm)
{
	...
	arm->edel = MEM_callocN(sizeof(ListBase), "edel armature");
	arm->act_edelement = make_elementList(arm->edel, &arm->elementbase, NULL, arm->act_element);
...
static void undoBones_to_editBones(void *uarmv, void *armv, void *UNUSED(data))
{
	...
	ED_armature_eelement_listbase_free(arm->edel);
	ED_armature_ebone_listbase_copy(arm->edel, &uarm->lb);
	...
	ED_armature_ebone_listbase_temp_clear(arm->edel);
}
...
static void *editBones_to_undoBones(void *armv, void *UNUSED(obdata))
{
	...
	ED_armature_ebone_listbase_copy(&uarm->lb, arm->edel);
