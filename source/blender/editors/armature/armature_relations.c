	...
	CTX_DATA_BEGIN(C, Base *, base, selected_editable_bases)
	{
		if ((base->object->type == OB_ARMATURE) && (base->object != ob)) {
			...
			for (pchan = opose->chanbase.first; pchan; pchan = pchann) {
				pchann = pchan->next;
				curbone = ED_armature_element_find_name(curarm->edel, pchan->name);

				/* Get new name */
				unique_editarmatureelement_name(arm->edel, curbone->name, NULL);
				...
				/* Jump Ship! */
				BLI_remlink(curarm->edel, curbone);
				BLI_addtail(arm->edel, curbone);
		...
		CTX_DATA_BEGIN(C, EditArmatureElement *, ebone, selected_editable_bones)
		{
			if (ELEM(ebone, actbone, actmirb) == 0) {
				if (ebone->flag & ELEMENT_SELECTED)
					bone_connect_to_new_parent(arm->edel, ebone, actbone, val);
				else
					bone_connect_to_new_parent(arm->edel, ebone, actmirb, val);
			}
		}
		CTX_DATA_END;
	}
