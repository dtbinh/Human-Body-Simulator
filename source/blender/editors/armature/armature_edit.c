	...
	if (arm->flag & ARM_MIRROR_EDIT) {
		for (eelement = arm->edel->first; eelement; eelement = eelement->next) {
			if ((EELEMENT_VISIBLE(arm, eelement) && EELEMENT_EDITABLE(eelement)) == 0) {
				EditArmatureElement *eelement_mirr = ED_armature_element_get_mirrored(arm->edel, eelement);
...
typedef struct EditBonePoint {
	...
	EditArmatureElement *head_owner;       /* EditArmatureElement which uses this point as a 'head' point */
	EditArmatureElement *tail_owner;       /* EditArmatureElement which uses this point as a 'tail' point */
...
static int armature_merge_exec(bContext *C, wmOperator *op)
{
	...
	if (type == 1) {
		...
		chains_find_tips(arm->edel, &chains);
...
static int armature_align_bones_exec(bContext *C, wmOperator *op)
{
	...
	if (actbone == NULL) {
		 ...
		 */
		actmirb = ED_armature_element_get_mirrored(arm->edel, actbone);
		if (actmirb == NULL)
			actmirb = actbone;
	}
	...
	if (num_selected_bones <= 1) {
		 ...
		 */
		if (actbone->parent) {
			bone_align_to_bone(arm->edel, actbone, actbone->parent);

			if ((arm->flag & ARM_MIRROR_EDIT) && (actmirb->parent))
				bone_align_to_bone(arm->edel, actmirb, actmirb->parent);
		...
		CTX_DATA_BEGIN(C, EditArmatureElement *, ebone, selected_editable_bones)
		{
			if (ELEM(ebone, actbone, actmirb) == 0) {
				if (ebone->flag & ELEMENT_SELECTED)
					bone_align_to_bone(arm->edel, ebone, actbone);
				else
					bone_align_to_bone(arm->edel, ebone, actmirb);
			}
		}
		CTX_DATA_END;
...
static int armature_split_exec(bContext *C, wmOperator *UNUSED(op))
{
	...
	for (bone = arm->edel->first; bone; bone = bone->next) {
...
//	EditBone *curBone, *ebone_next;
	...
	if (obedit->pose) {
		...
		for (pchan = obedit->pose->chanbase.first; pchan; pchan = pchan_next) {
			...
			curBone = ED_armature_element_find_name(arm->edel, pchan->name);
				...
				for (con = pchan->constraints.first; con; con = con->next) {
					...
					if (cti && cti->get_constraint_targets) {
						...
						for (ct = targets.first; ct; ct = ct->next) {
							if (ct->tar == obedit) {
								if (ct->subtarget[0]) {
									curBone = ED_armature_element_find_name(arm->edel, ct->subtarget);
									if (curBone && (curBone->flag & ELEMENT_SELECTED) && (arm->layer & curBone->layer)) {
	...
	for (curBone = arm->edel->first; curBone; curBone = ebone_next) {
