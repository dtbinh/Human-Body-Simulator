		...
		for (curcon = conlist->first; curcon; curcon = curcon->next) {
			...
			if (curcon->type == CONSTRAINT_TYPE_KINEMATIC) {
				...
				if (BKE_object_exists_check(data->tar) == 0) {
					...
					if (!BKE_armature_find_element_name(BKE_armature_from_object(owner), data->subtarget)) {
						curcon->flag |= CONSTRAINT_DISABLE;
					}
				}
				...
				if (data->poletar) {
					if (BKE_object_exists_check(data->poletar) == 0) {
						...
						if (!BKE_armature_find_element_name(BKE_armature_from_object(owner), data->polesubtarget)) {
							curcon->flag |= CONSTRAINT_DISABLE;
						}
					}
				}
				...
				for (ct = targets.first; ct; ct = ct->next) {
					...
					if (BKE_object_exists_check(ct->tar) == 0) {
						...
						if (type == CONSTRAINT_OBTYPE_BONE) {
							if (!BKE_armature_find_element_name(BKE_armature_from_object(owner), ct->subtarget)) {
